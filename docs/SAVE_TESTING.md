...
> 测试日期：2026-08-25　|　测试人：Agent（DSH）　|　测试方式：客户端 quickPlay 自动进入存档 + 日志检查
...
`run/saves/` 下共 31 个存档，**排除所有"新的世界"系列**（`新的世界`、`新的世界 (x)`、`新的世界2 (1)`），即跳过默认/复制生成的空白存档，只测试 22 个有具体名称的场景存档。
...
> 注：2026-08-25 重跑全部 22 个场景存档（较上次新增 工厂芯片开销测试），加载耗时以各存档日志实测为准；"建筑服场景" 的 level.dat 显示名为"新的世界"，以文件夹写入时间确认加载正确。

**结论：ether_craft 在全部 22 个场景存档中加载与运行均无崩溃、无异常。** 压力测试类存档（以太流/工厂系列，含新增 工厂芯片开销测试）中的 block entity tick 在观察期正常运转，capabilities、发射器链传输、碰撞、红石、携带实体等场景均正常。
