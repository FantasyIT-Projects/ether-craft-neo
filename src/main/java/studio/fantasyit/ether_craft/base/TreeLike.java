...
public class TreeLike<T, U> {
    public static class TreeNode<T, U> {
        ...
        public TreeEdge<T, U> reverseEdge;
    ...
    public TreeLike<T, U> addEdge(Integer from, Integer to, U value) {
        ...
        if (toNode.reverseEdge != null) {
            toNode.reverseEdge.from.edges.remove(toNode.reverseEdge);
        }
        TreeEdge<T, U> tuTreeEdge = new TreeEdge<>(value, toNode, fromNode);
        fromNode.edges.add(tuTreeEdge);
        toNode.reverseEdge = tuTreeEdge;
        return this;
    }
    ...
    public void removeSubTree(TreeNode<T, U> node) {
        node.edges.forEach(t -> removeSubTree(t.node));
        idMapping.remove(node.id);
    }
}